...
import { X, Check, Building2, CreditCard, Wallet, Plus } from "lucide-react"
...
const SUGGESTED_AMOUNTS = [1000, 5000, 10000, 25000, 50000, 100000]
            ...
            >
              ...
              <div className="min-h-[240px]">
                ...
                {activeField === "amount" && (
                  ...
                  >
                    {/* Big amount input */}
                    <div className="flex items-center gap-2 bg-white/6 rounded-2xl px-4 py-3 border border-white/10 focus-within:border-[#7EC8A4]/50 transition-colors">
                      <span className="text-2xl font-bold text-[#7EC8A4]">₹</span>
                      <input
                        ref={amountInputRef}
                        type="number"
                        inputMode="decimal"
                ...
                {activeField === "title" && (
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-2 mt-3 mb-1">
                      {["Salary", "Freelance", "Refund", "Gift", "Interest", "Sale"].map((label) => (
                        <button
                          key={label}
                          onClick={() => {
                  ...
                  >
                    ...
                    {categoriesLoading ? (
                          ...
                          return (
                            <button
                              ...
                              className={`relative flex flex-col items-center gap-1.5 py-3 rounded-2xl border text-center transition-all active:scale-95
                                ${selected
                                  ? "bg-[#5B9CF6]/15 border-[#5B9CF6]/40"
                                  : "bg-white/5 border-white/8"
                                }`}
                            >
                              {selected && (
                                <span className="absolute top-1.5 right-1.5 w-3.5 h-3.5 bg-[#5B9CF6] rounded-full flex items-center justify-center">
                              ...
                              <span className={`text-[10px] font-semibold leading-tight truncate w-full text-center px-1
                                ${selected ? "text-[#5B9CF6]" : "text-white/50"}`}>
                                {cat.name}
                              </span>
                            </button>
                          )
                      ...
                      <div className="bg-white/5 border border-white/8 rounded-2xl py-6 text-center">
                        <p className="text-xs text-white/30">No income categories yet</p>
                        <AddCategory
                          trigger={
                            <button className="mt-2 text-xs font-semibold text-[#5B9CF6] flex items-center gap-1 mx-auto">
                          ...
                          return (
                            <button
                              ...
                              className={`flex items-center gap-2 px-3 py-3 rounded-2xl border transition-all active:scale-95
                                ${selected
                                  ? "bg-[#E8A44A]/15 border-[#E8A44A]/40 text-[#E8A44A]"
                                  : "bg-white/5 border-white/8 text-white/60"
                                }`}
                            >
                              <span className={selected ? "text-[#E8A44A]" : "text-white/40"}>
                                {accountIcon(acc.type)}
                              </span>
                              <span className="text-sm font-semibold truncate">{acc.name}</span>
                              {selected && <Check size={13} className="ml-auto text-[#E8A44A] shrink-0" strokeWidth={2.5} />}
                            </button>
                          )
                        })}
                      </div>
