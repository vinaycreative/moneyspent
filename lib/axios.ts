...
api.interceptors.request.use(
  async (config: InternalAxiosRequestConfig) => {
    ...
    console.log('With credentials enabled:', config.withCredentials)
    ...
    // We rely on cookies for authentication, no need to set Authorization header
    // The withCredentials: true setting ensures cookies are sent
    ...
    if (error.response?.status === 401 && originalRequest && !originalRequest._isRetry) {
      ...
      try {
        ...
        if (authRecovered) {
          ...
          // Cookie will be automatically updated by the auth manager
